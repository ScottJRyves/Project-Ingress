def cleaning_url_list(urls):
    url_list = []

    for url in urls.splitlines():  # splits the urls in the textarea at the end of each line
        url = url.strip()  # strips any whitespace from the beginning and end of the url
        if url:  # basically this means if the url is not empty, then add it to the list
            url_list.append(url)

    url_list = list(set(url_list))  # removes any duplicates from the list of urls

    return url_list


# Was having an issue with stopwords not exactly getting everything so made a custom,
# was able to find a lot of these on this page
# Reference:
